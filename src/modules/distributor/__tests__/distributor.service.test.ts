...
describe('DistributorService', () => {
  ...
  describe('getDashboard', () => {
    it('should return dashboard data successfully', async () => {
      const mockData = [
        { totalStock: 1000, totalInbound: 500, totalOutgoing: 200 },
        [{ KirimanId: '123', JenisPupuk: 'Urea', JumlahDikirimKg: 100, TanggalPengiriman: null, StatusPengiriman: 'Dikirim' }],
        [{ pupukId: 1, jenisPupuk: 'Urea', jumlah: 900, status: 'Aman' }],
        [{ NotifikasiId: 'notif-1', JudulNotifikasi: 'Test', PesanNotifikasi: 'Test message', TanggalNotifikasi: null }]
      ];
      ...
      expect(result).toEqual({
        ...
        recentShipments: [{ kirimanId: '123', jenisPupuk: 'Urea', jumlahDikirim: 100, timestampDikirim: null, status: 'Dikirim' }],
        recentStockOut: [{ pupukId: 1, jenisPupuk: 'Urea', jumlah: 900, status: 'Aman' }],
        notifications: [{ notifikasiId: 'notif-1', judul: 'Test', pesan: 'Test message', timestamp: null }],
      });
    ...
    it('should throw AppError on database failure', async () => {
      ...
      await expect(service.getDashboard(123)).rejects.toThrow(AppError);
      await expect(service.getDashboard(123)).rejects.toThrow('Gagal mengambil data dashboard');
    });
  });
  ...
  describe('validatePengecer', () => {
    it('should validate pengecer successfully', async () => {
      ...
      const result = await service.validatePengecer(123);
    ...
    it('should throw AppError when pengecer is invalid', async () => {
      ...
      await expect(service.validatePengecer(123)).rejects.toThrow(AppError);
      await expect(service.validatePengecer(123)).rejects.toThrow('User tidak aktif');
    });
  });
  ...
  describe('getStockDashboard', () => {
    it('should return stock dashboard with pagination', async () => {
      const mockData = [
        { totalStockTon: 1.5 },
        [
          { pupukId: 1, jenisPupuk: 'Urea', jumlah: 1000, lastUpdated: null },
          { pupukId: 2, jenisPupuk: 'NPK', jumlah: 500, lastUpdated: null }
        ]
      ];
      ...
      const result = await service.getStockDashboard(123, 1);
  ...
  describe('markNotificationRead', () => {
    it('should mark notification as read successfully', async () => {
      ...
      const result = await service.markNotificationRead(123, 123);
    ...
    it('should handle errors when marking notification as read', async () => {
      ...
      await expect(
        service.markNotificationRead(123, 123)
