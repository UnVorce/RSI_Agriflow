...
export class PetaniService {
  ...
  async getFarmerById(petaniId: string, userIdPengecer: string) {
    try {
  ...
  async getFarmerQuota(petaniId: string, userIdPengecer: string) {
    try {
  ...
  async deleteFarmer(petaniId: string, userIdPengecer: string) {
    try {
      // Verify farmer belongs to retailer
      const existing = await prisma.petani.findFirst({
        where: {
