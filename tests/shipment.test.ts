...
describe('Shipment Module Tests', () => {
  ...
  beforeAll(async () => {
    ...
    if (!pupuk) {
      pupuk = await prisma.pupuk.create({
        data: { PupukId: Math.floor(Math.random() * 1000) + 10, JenisPupuk: 'NPK' },
      });
    }
  ...
  describe('Shipment History', () => {
    ...
    it('should filter history by status', async () => {
      ...
      const filtered = history.filter(h => h.Status === 'Diterima');
      expect(filtered.every((h) => h.Status === 'Diterima')).toBe(true);
    });
  });
});
