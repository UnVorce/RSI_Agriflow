...
describe('Redemption Module Tests', () => {
  ...
  beforeAll(async () => {
    ...
    const petaniId = '1234567890123456';
  ...
  describe('BR-01: Farmer Validation', () => {
    ...
    it('should reject farmer ID with wrong length', async () => {
      ...
      ).rejects.toThrow('PetaniId harus 16 digit');
    });
    ...
    it('should reject farmer not belonging to retailer', async () => {
      ...
      ).rejects.toThrow('Petani tidak terdaftar');
    });
    ...
    it('should check registration period', async () => {
      ...
      expect(result.petani.AwalTerdaftar).toBeDefined();
      expect(result.petani.AkhirTerdaftar).toBeDefined();
    });
  });
    ...
    it('should reject when stock is insufficient', async () => {
      await expect(
        redemptionService.redeemFertilizer({
          ...
          petaniId: testPetaniId,
          pupukId: testPupukId,
          jumlah: 10000, // More than available
        })
    ...
    it('should reject when quota is insufficient', async () => {
      await expect(
        redemptionService.redeemFertilizer({
          ...
          petaniId: testPetaniId,
          pupukId: testPupukId,
          jumlah: 200, // More than quota
        })
      ).rejects.toThrow('Kuota tidak mencukupi');
    });
    ...
    it('should not allow negative quota', async () => {
      ...
      await expect(
        redemptionService.redeemFertilizer({
          ...
          petaniId: testPetaniId,
          pupukId: testPupukId,
          jumlah: quota!.SisaKuota.toNumber() + 10,
        })
      ).rejects.toThrow('Kuota tidak mencukupi');
    });
  });
